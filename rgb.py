...
class ColorHelper(object):
    ...
    def hexToGreen(self, hex):
        """
        Parses a valid hex color string and returns the Green RGB
        integer value.
        """
    ...
    def randomRGBValue(self):
        """
        Return a random Integer in the range of 0 to 255, representing an
        RGB color value.
        """
    ...
    def getClosestPointToLine(self, A, B, P):
        """
        Find the closest point on a line. This point will be reproducible
        by a Hue lamp.
        """
    ...
    def getRGBFromXYAndBrightness(self, x, y, bri=1):
        """
        Returns a rgb tuplet for given x, y values.  Not actually an inverse
        of `getXYPointFromRGB`.

        Implementation of the instructions found on the Philips Hue
        iOS SDK docs: http://goo.gl/kWKXKl
        """
        xyPoint = XYPoint(x, y)
...
class Converter(object):
    ...
    def getCIEColor(self, hexColor=None):
        """
        Returns the approximate CIE 1931 x, y coordinates represented by
        the supplied hexColor parameter, or of a random color if the
        parameter is not passed. The point of this function is to let people
        set a lamp's color to any random color. Arguably this should be
